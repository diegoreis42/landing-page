...
export const Carousel = ({ serviceList }: ServiceList) => {
  ...
  serviceList.forEach((service, idx) => {
    ...
    if (
      internalService.length === NUMBER_PER_PAGE ||
      idx === serviceList.length - 1
    ) {
  ...
  return (
    <BootstrapCarousel
      ...
      className="w-screen bg-sky-700 pt-15"
    >
        ...
        <BootstrapCarousel.Item key={idx} className="items-center p-12">
          <div className="flex justify-evenly">
            {list.map((service: Service, idx) => (
              <Card
                key={idx}
                className="w-[20rem] shadow-lg hover:bg-slate-200"
              >
                <Card.Img variant="top" src={service.image.path} />
                <Card.Body>
                  <Card.Title>{service.mainText}</Card.Title>
                  <Card.Text>{service.subText}</Card.Text>
                  <Button variant="primary" className="hover:bg-blue-500">
                    Contratar
                  </Button>
                </Card.Body>
              </Card>
